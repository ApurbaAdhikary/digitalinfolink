  ...
  var polyfilter = {
    ...
    _development_mode: (function () {
      if (
        ...
        location.hostname.search(/\d+\.\d+\.\d+\.\d+/) !== -1
      ) {
        if (window.console)
          console.log(
    ...
    process_stylesheets: function () {
      ...
      window.setTimeout(function () {
        ...
        xmlHttpCheck.open(
          "GET",
          window.polyfilter_scriptpath + "htc/sepia.htc",
          true
        );
        xmlHttpCheck.onreadystatechange = function () {
          if (xmlHttp.readyState == 4 && xmlHttp.status != 200) {
            alert(
              'The configured path \r\rvar polyfilter_scriptpath = "' +
                window.polyfilter_scriptpath +
          ...
          switch (stylesheets[i].nodeName) {
            ...
            case "LINK":
              if (stylesheets[i].rel === "stylesheet") {
                ...
                if (
                  !polyfilter._development_mode &&
                  window.localStorage &&
                  window.localStorage.getItem("polyfilter_" + href)
                ) {
                  polyfilter._pending_stylesheets--;
                  polyfilter._stylesheets[index].content = localStorage.getItem(
                    "polyfilter_" + href
                  );
                  ...
                  xmlHttp.onreadystatechange = function () {
                    if (xmlHttp.readyState === 4) {
                      if (xmlHttp.status === 0) {
                        if (window.console)
                          console.log(
                            "Could not fetch external CSS via HTTP-Request " +
                              href +
                              ". Probably because of cross origin."
                          );
                        if (!polyfilter._stylesheets[index].content) {
                          polyfilter._pending_stylesheets--;
                          polyfilter._stylesheets[index].content =
                        ...
                        if (
                          ...
                          try {
                            ...
                            if (window.console)
                              console.log(
                                "Local storage quota have been exceeded. Caching of stylesheet " +
                                  href +
                                  " is not possible"
                              );
                          }
      ...
      for (var k in rule.declarations) {
        ...
        if (declaration.property === "filter") {
          ...
          for (idx in values) {
            ...
            for (key in currentproperties) {
              if (typeof properties[key] !== "undefined") {
                properties[key] = properties[key].concat(
          ...
          if (properties["filtersW3C"].length > 0) {
            var filter = (webkitFilter = mozFilter = oFilter = msFilter = properties[
              "filtersW3C"
            ].join(" "));

            if (
              properties["filtersWebKit"] &&
              properties["filtersWebKit"].length > 0
            ) {
          ...
          if (properties["filtersSVG"].length > 0) {
            if (properties["filtersSVG"][0] != "none") {
              ...
              if (typeof this._svg_cache[id] === "undefined") {
                this._svg_cache[id] = this._create_svg(
                  id,
                  properties["filtersSVG"]
                );
    ...
    scriptpath: window.polyfilter_scriptpath
      ? window.polyfilter_scriptpath
      : (function () {
          alert(
        ...
        Object.defineProperty(CSSStyleDeclaration.prototype, "polyfilter", {
          ...
          set: function (gluedvalues) {
            ...
            if (properties["filtersW3C"].length > 0) {
              ...
              this.webkitFilter = this.mozFilter = this.oFilter = properties[
                "filtersW3C"
              ].join(" ");
            }
            ...
            if (properties["filtersSVG"].length > 0) {
              if (properties["filtersSVG"][0] != "none") {
                ...
                if (typeof polyfilter._svg_cache[id] === "undefined") {
                  polyfilter._svg_cache[id] = polyfilter._create_svg(
                    id,
                    properties["filtersSVG"]
                  );
                  ...
                  if (typeof XMLSerializer === "undefined") {
                    ...
                    var s = new XMLSerializer();
                    var svgString = s.serializeToString(
                      polyfilter._svg_cache[id]
                    );
                    if (svgString.search("SourceGraphic") != -1) {
                      document.body.appendChild(polyfilter._svg_cache[id]);
                    }
    ...
    convert: function (value) {
      ...
      var fmatch = value.match(
        /(drop\-shadow)\(([0-9]+)[px]*\s+([0-9]+)[px]*\s+([0-9]+)[px]*\s+([#0-9]+)\)/i
      );
      if (fmatch !== null) {
    ...
    filters: {
        ...
        if (typeof polyfilter._ie === "undefined") {
          ...
          properties["filtersIE"] =
            amount >= 0.5
              ...
              : [];
          properties["behaviorsIE"] =
            amount >= 0.5
              ? ['url("' + polyfilter.scriptpath + 'htc/sepia.htc")']
              : [];
        }
      ...
      brightness: function (amount) {
        ...
        if (typeof polyfilter._ie === "undefined") {
          ...
          properties["behaviorsIE"] = [
            'url("' + polyfilter.scriptpath + 'htc/brightness.htc")',
          ];
        }
      ...
      dropShadow: function (offsetX, offsetY, radius, color) {
        ...
        if (typeof polyfilter._ie === "undefined") {
          ...
          // IE
          properties["filtersIE"] = [
            "progid:DXImageTransform.Microsoft.Glow(color=" +
              color +
              ",strength=0)",
          ...
          properties["behaviorsIE"] = [
            'url("' + polyfilter.scriptpath + 'htc/drop-shadow.htc")',
          ];
        }
